...
- **packages/llm** — Agent engine. Agentic loop with tool registry (Zod-validated), streaming (`streamText`), parallel tool execution (`Promise.allSettled`), trace event pipeline, model-aware cost calculation with cached token discount. 56 tests (Vitest, 7 files). Amygdala in `src/amygdala/index.ts` (createAmygdala factory, generateObject with substrate-aware system prompt, 4 trace event types). Subagent registry in `src/subagent/registry.ts` (5 configs: resume, project, security, conversational, refusal). Scoped tool registry via `createScopedRegistry()`. **Orchestrator** in `src/orchestrator/index.ts` (createOrchestrator factory — deterministic routing: AmygdalaResult → subagent selection → context filtering → scoped tool access → agent() invocation. Emits `orchestrator:route` and `orchestrator:context-filter` trace events. Threat override at ≥0.8 forces refusal regardless of intent.) **LLM-as-Judge** in `src/eval/judge.ts` (createJudge factory — generateObject with Haiku 4.5, 3-dimension rubric (relevance/safety/helpfulness, 1-5 scale), runtime score clamping, never-throw error handling. Gated by `ENABLE_LLM_JUDGE` env var. Emits `eval:score` trace event.)
