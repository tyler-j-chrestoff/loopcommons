...
- **packages/web** — Next.js frontend. `route.ts` is a thin HTTP adapter assembling ToolPackages (resume, project, blog, memory, arena-query) and delegating to `agentCore.invoke()`. Chat UI with SSE streaming, amygdala/routing/memory inspectors, blog CMS (8 tools, auth-gated), session persistence (JSONL), arena/tournament viz. `/arena` page with live tournament streaming (SSE), fitness charts, population grid, encounter results heatmap. Tournament API: POST to start, GET SSE stream, GET state snapshot, GET current. Tournament persistence: JSONL per-tournament in `data/arena/tournaments/{id}/` (generations, events, step traces per agent per encounter), rehydrated on cold start. Chat CLI in `scripts/chat.ts`. ~530 tests.
