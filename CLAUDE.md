...
- **packages/web** — Next.js frontend. `route.ts` is a thin HTTP adapter assembling ToolPackages (resume, project, blog, memory, arena-query) and delegating to `agentCore.invoke()`. Chat UI with SSE streaming, amygdala/routing/memory inspectors, blog CMS (8 tools, auth-gated), session persistence (JSONL), arena/tournament viz. `/arena` page: results-first layout with `EncounterHeatmap` (agent × encounter grid, color-coded scores, clickable cells), `FeaturedDeath` card (auto-selected most interesting death with epitaph), past tournaments list; live tournament streaming (SSE), fitness charts, population grid. Tournament browser API: GET list + GET detail from disk. Tournament control API: POST to start, GET SSE stream, GET state snapshot, GET current. `tournament-loader.ts` reads persisted JSONL from `data/arena/tournaments/{id}/`. Tournament persistence: JSONL per-tournament (generations, events, step traces per agent per encounter), rehydrated on cold start. Chat CLI in `scripts/chat.ts`. ~560 tests.
