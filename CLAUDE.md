...
- **packages/llm** — Agent engine. Agentic loop with tool registry (Zod-validated), streaming (`streamText`), parallel tool execution (`Promise.allSettled`), trace event pipeline, model-aware cost calculation with cached token discount. 266 tests (Vitest, 19 files — 4 unit, 4 red-team, 3 eval, 1 smoke, 2 blog, 5 calibration). Amygdala in `src/amygdala/index.ts` (createAmygdala factory, generateObject with substrate-aware system prompt, 4 trace event types). Blog types in `src/blog/types.ts` (BlogPost, BlogPostSummary, BlogFrontmatterSchema, SLUG_REGEX). Subagent registry in `src/subagent/registry.ts` (7 configs: resume, project, blog-reader, blog-writer, security, conversational, refusal). Registry `get()` accepts optional `RoutingContext` for auth-dependent routing (blog intent → blog-reader or blog-writer based on `isAdmin`). Scoped tool registry via `createScopedRegistry()`. **Orchestrator** in `src/orchestrator/index.ts` (createOrchestrator factory — deterministic routing: AmygdalaResult → subagent selection → context filtering → scoped tool access → agent() invocation. Emits `orchestrator:route` (with `authGated` field) and `orchestrator:context-filter` trace events. Threat override at ≥0.8 forces refusal regardless of intent. Accepts `isAdmin` in `OrchestratorInput` for auth-gated blog routing.) **LLM-as-Judge** in `src/eval/judge.ts` (createJudge factory — generateObject with Haiku 4.5, 3-dimension rubric (relevance/safety/helpfulness, 1-5 scale), runtime score clamping, never-throw error handling. Gated by `ENABLE_LLM_JUDGE` env var. Emits `eval:score` trace event.) **CI Eval Suite** in `test/eval-{quality,safety,routing}.test.ts` (130 tests — response quality, safety classification, routing correctness incl. blog auth-gating assertions. Mock mode for CI (no API key), fixture-driven via `test/fixtures/eval-cases.json` (24 fixtures incl. 8 blog-specific). Threshold-based: detection >= 90%, FP <= 10%, intent accuracy >= 85%). **Red-team blog** in `test/red-team-blog.test.ts` (12 tests — 6 amygdala API attacks targeting blog write tools + 6 deterministic routing assertions for blog auth gating). **Calibration split** in `test/fixtures/calibration-split.json` (12 optimize / 6 holdout for auto-calibration runner). **Auto-calibration** in `src/calibration/` (4 modules: `proposer.ts` — generateObject with Haiku 4.5 meta-prompt, `memory.ts` — typed StructuredMemory (Observation/Learning/Reflection/Experience) with JSON persistence and deduplication, `fitness.ts` — multi-metric fitness evaluator with Pareto constraint, `logger.ts` — JSONL atomic append with fsync). **Runner** in `src/calibration/runner.ts` (propose-apply-test-evaluate-keep/revert loop, dependency-injected proposer + testRunner, applyEdit/readPromptFromFile/writePromptToFile helpers, stopping criteria: 5 no-improvement or 50 max iterations). CLI in `scripts/calibrate.ts` (`npm run calibrate`). 77 calibration tests across 5 files. **Live eval** in `test/eval-live.test.ts` (19 tests, gated by `EVAL_LIVE=true` + API key — real API calls, threshold assertions. Tests "safely handled" = refused OR zero-tool routed. Run locally via `npm run test:eval:live`).
- **packages/web** — Next.js frontend. Chat UI with SSE streaming (`text-delta` events for token-by-token display). **Full amygdala pipeline in route.ts**: amygdala → orchestrator → subagent (replaces direct agent() call). **Auth**: NextAuth.js v5 middleware (`src/middleware.ts`) redirects to `/login`; credentials provider with JWT sessions; chat API checks session; sessions/metrics APIs accept session or X-API-Key. **Event sanitization** in `src/lib/sanitize-event.ts` (shared module: `sanitizeEvent` for SSE, `sanitizeSessionEvent` for API — strips rawResponse headers + system prompts). **Token budget tracking**: `TokenBudgetAccumulator` in `src/lib/token-budget.ts` — per-request cumulative token tracking across sources (amygdala/subagent), budget percent vs 200K context limit, cost estimation with Haiku 4.5 pricing (cache read 0.1x, cache creation 1.25x). Emits `token-budget:update` SSE event with full snapshot. Client state in `use-chat.ts`. **Context budget viz**: ContextBudgetBar (persistent fill bar — green/yellow/orange/red thresholds at 75/90/100%, pulse at 90%+, shimmer during streaming, clickable per-turn breakdown via TokenBreakdown). TokenBreakdown (per-turn: source, input/output tokens, cache indicator, cost). **Viz components**: AmygdalaInspector (composer — PipelineTimeline + AmygdalaPassCard + RoutingCard with stage-click interaction), PipelineTimeline (horizontal 4-stage flow with latency + threat glow), AmygdalaPassCard (collapsible: intent badge, threat gauge, rewrite diff, reasoning), RoutingCard (subagent badge, tool tags, context filtering stats). Also: TraceInspector, CostDashboard, ToolCallInline. 10 tools: `get_resume`, `get_project`, 8 blog tools via `createBlogTools` factory (`list_posts`, `read_post`, `create_draft`, `edit_post`, `publish_post`, `unpublish_post`, `delete_post`, `list_drafts`). **Blog storage** in `src/lib/blog/store.ts` (file-based CRUD: markdown + YAML frontmatter via gray-matter, drafts/published dirs, slug validation, path traversal prevention, `BLOG_DATA_DIR` env var). Blog tools in `src/tools/blog.ts` (factory returns 8 `defineTool` instances backed by `BlogStore`). **Auth-gated routing**: route.ts passes `isAdmin` (from `session.user.name === 'Admin'`) to orchestrator → blog-reader (public, 2 read tools) or blog-writer (admin, all 8 tools). Rate limiting and concurrency guard in `src/lib/rate-limit.ts`. **Session persistence**: FileSessionWriter writes all events to JSONL in `data/sessions/{date}/{id}.jsonl` (reads `SESSION_DATA_DIR` env var, defaults to `data/sessions/`). Session ID in header + `session:start` SSE event. **Session API**: `GET /api/sessions` (list, paginated), `GET /api/sessions/[id]` (events, sanitized). **Session CLI**: `scripts/session.ts` (list/read with TTY colors). **Session UI**: session ID + Export JSON button in chat header + SessionThread (collapsible linked conversation chain). **Session linking**: `parentSessionId` field on `session:start` events and `SessionSummary`. Client stores last session ID in localStorage, sends as `X-Parent-Session-Id` header. `GET /api/sessions?thread=<id>` walks links to build full thread. 348 tests (Vitest, 29 files): token-budget, sanitize-event, rate-limit, spend-tracker, sanitize, file-session-writer, red-team-sessions, session-linking, format, api-sessions, api-metrics, api-chat, api-feedback, use-chat, feedback, feedback-buttons, feedback-badge, judge-score-card, components-chat, components-observability, components-amygdala, context-budget-bar, token-breakdown, blog-store, blog-tools, blog-markdown, blog-data, blog-pages, blog-inaugural. **Blog frontend** in `src/app/blog/` — `/blog` listing page (Server Component, post cards), `/blog/[slug]` post page (Server Component, rendered markdown, OG metadata, 404). Blog layout with nav (Blog/Chat links). `/blog` routes public via middleware exemption. **Markdown rendering** in `src/lib/blog/markdown.ts` (unified pipeline: remark-parse → remark-gfm → remark-rehype → rehype-pretty-code with github-dark-dimmed Shiki theme → rehype-stringify). **Blog data access** in `src/lib/blog/data.ts` (getPublishedPosts, getPost — server-side, backed by BlogStore + renderMarkdown). **Blog prose styling** in globals.css (.blog-prose — custom dark theme styles for headings, code, tables, blockquotes, links). **User feedback**: FeedbackButtons (thumbs up/down + category picker on hover), FeedbackBadge (post-submission indicator), POST /api/feedback (appends eval:feedback to session JSONL), submitFeedback in use-chat.ts (optimistic UI). **LLM-as-judge viz**: JudgeScoreCard (3 horizontal score bars, color-coded, collapsible reasoning). Judge scores stored on ChatMessage via eval:score SSE event.
...
- **Layer 2 (amygdala, integrated)**: Metacognitive security layer — no tool access, substrate-aware system prompt, rewrite-as-compression (REMOVE only, never fabricate), intent classification, threat assessment, context delegation. Full pipeline in route.ts: `createAmygdala` → `createOrchestrator` → scoped subagent. Threat override at ≥0.8 forces refusal. Amygdala cost now tracked in spend tracker. **Consciousness research carve-out** (session 14): amygdala prompt distinguishes philosophy-as-topic (on-topic, safe) from philosophy-as-lever (adversarial). **Rewrite guard** (session 14): runtime check in route.ts catches amygdala returning a history message as the rewrite instead of the current message.
