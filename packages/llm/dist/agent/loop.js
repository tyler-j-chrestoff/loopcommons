...
function emit(collectors, event) {
...
async function callProvider(provider, params, tools, collectors, useStreaming) {
    const callParams = {
        model: params.model,
    ...
    if (useStreaming && provider.streamCall) {
        ...
        if (!result)
            throw new LLMError('PROVIDER_ERROR', 'Stream ended without finish event');
        return result;
    }
