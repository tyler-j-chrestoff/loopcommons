...
export type Round = {
  ...
  completedAt: number;
  latencyMs: number;
  request: { messages: Message[]; toolNames: string[] };
  response: { content: string; toolCalls: ToolCall[]; usage: TokenUsage; cost: number; finishReason: string; rawResponse: unknown };
