...
{"id":"blog-13","story":"blog-routing","description":"Write routing tests (TDD). Test: intent=blog with isAdmin=true → blog-writer subagent with all 8 tools. intent=blog with isAdmin=false → blog-reader subagent with 2 tools only. Threat override at >=0.8 still forces refusal regardless of blog intent. Auth-gating trace event emitted. Existing intent routing unchanged (resume, project, etc. ignore isAdmin). Non-admin never gets create_draft, edit_post, publish_post, unpublish_post, delete_post, or list_drafts.","depends_on":["blog-11"],"requires":"","status":"done"}
```
